...
trait Validate
{
    ...
    {
        $errors = []; ///$this->language->feedKey('text_error_' . 'min' , [$this->language->get('text_label_' . 'logo') , 1 , 5]);
        if (!empty($roles)) {
                ...
                foreach ($validationRoles as $validationRole) {
                    ...
                    if (preg_match_all('/(min)\((\d+)\)/', $validationRole, $m)) {
                        if ($this->min($value, $m[2][0]) === false) {
                            if ($this->isajax){
                                $errors[] = $this->language->feedKey('text_error_' . $m[1][0], [$this->language->get('text_label_' . $fieldName), $m[2][0]]);
                                continue;
                            }
                            $this->messenger->add(
                            ...
                            $e = $fieldName . '  min  ' . $value;
                            $this->messenger->add($e, Messenger::APP_MESSAGE_ERROR);
                        ...
                        if ($this->between($value, $m[2][0], $m[3][0]) === false) {
                            if ($this->isajax){
                                $errors[] = $this->language->feedKey('text_error_' . $m[1][0], [$this->language->get('text_label_' . $fieldName), $m[2][0], $m[3][0]]);
                                continue;
                            }
                            $this->messenger->add(
                        ...
                        if ($this->eq_field($value, $otherFieldValue) === false) {
                            if ($this->isajax){
                                $errors[] = $this->language->feedKey('text_error_' . $m[1][0], [$this->language->get('text_label_' . $fieldName), $this->language->get('text_label_' . $m[2][0])]);
                                continue;
                            }
                            $this->messenger->add(
                        ...
                        if ($this->$validationRole($value) === false) {
                            if ($this->isajax){
                                $errors[] = $this->language->feedKey('text_error_' . $validationRole , [$this->language->get('text_label_' . $fieldName)]);
                                continue;
                            }
